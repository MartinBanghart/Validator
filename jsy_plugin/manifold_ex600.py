...
# Input for Parser
JJ5SY_PLUGIN_MFLD_EX600_TOKEN_MAP = [
    ...
    {"name": "ex600", "pattern": r"S6", "length": 2},
    {"name": "si_unit", "pattern": r"(WS|FB|FA|DA|EB|V|N|Q|0)", "length": None},
    {"name": "si_unit_polarity", "pattern": r"[2-9]?", "length": None},
    {"name": "io_stations", "pattern": r"[1-9]?", "length": None},
    {"name": "separator", "pattern": r"-", "length": 1},

    {"name": "valve_stations", "pattern": r"(02|03|04|05|06|07|08|09|10|11|12|13|14|15|16)", "length": 2},
    {"name": "p_e_port_entry_sup_exh", "pattern": r"[UDBCEF]", "length": 1},
    {"name": "separator", "pattern": r"-", "length": 1},
...
class JJ5SY_PLUGIN_MFLD_EX600_MODEL(BaseModel):
    # How to Order Information
    prefix: Literal['JJ5SY']
    series: Literal['1', '3', '5']
    type: Literal['10']
    ex600: Literal['S6']
    si_unit: Literal['0', 'Q', 'N', 'V', 'EB', 'DA', 'FA', 'FB', 'WS']
    si_unit_polarity: Literal['', '2', '3', '4', '5', '6', '7', '8', '9'] = ''
    io_stations: Literal['', '1', '2', '3', '4', '5', '6', '7', '8', '9'] = ''
    valve_stations: Literal['02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12', '13', '14', '15', '16']
    p_e_port_entry_sup_exh: Literal['U', 'D', 'B', 'C', 'E', 'F']
    ...
    def check_conditions(self) -> 'JJ5SY_PLUGIN_MFLD_EX600_MODEL':
        ...
        if self.p_e_port_entry_sup_exh in ('U', 'D', 'C', 'E') and self.valve_stations in ('11', '12', '13', '14', '15', '16'):
            raise ValueError('P,E Port entry cannot support this many valve stations')
        if self.si_unit =='0' and self.io_stations != '':
            raise ValueError('I/O stations cannot be selected without SI unit')
        if (self.si_unit == '0' and self.si_unit_polarity != '') or (self.si_unit != '0' and self.si_unit_polarity == ''):
            raise ValueError('If SI unit is selected so must be SI unit polarity. Otherwise, if SI unit is not selected, so must SI unit polarity')
    ...
    def build_part_number(self) -> str:
        return (
            ...
            f"-{self.type}{self.ex600}{self.si_unit}{self.si_unit_polarity}{self.io_stations}"
            f"-{self.valve_stations}{self.p_e_port_entry_sup_exh}"
            f"-{self.a_b_port_size}{self.mounting}{self.din_rail_opt}"
        )

    def description(self) -> str:
        return (
            f"JSY{self.series}000 {self.valve_stations} STA EX600 MANIFOLD"
        )
...
#     # ------------------------------------------- MANIFOLD BLOCK -------------------------------------------    
#     _manifold_block: 'JJ5SY_PLUGIN_MFLD_EX600_MODEL.Manifold_Block' = PrivateAttr()
...
#     @property
#     def manifold_block(self) -> 'JJ5SY_PLUGIN_MFLD_EX600_MODEL.Manifold_Block':
...
#     _sup_exh_block_d_side: 'JJ5SY_PLUGIN_MFLD_EX600_MODEL.Sup_Exh_Block_D_Side' = PrivateAttr()
...
#     @property
#     def sup_exh_block_d_side(self) -> 'JJ5SY_PLUGIN_MFLD_EX600_MODEL.Sup_Exh_Block_D_Side':
...
#     _sup_exh_block_u_side: 'JJ5SY_PLUGIN_MFLD_EX600_MODEL.Sup_Exh_Block_U_Side' = PrivateAttr()
...
#     @property
#     def sup_exh_block_u_side(self) -> 'JJ5SY_PLUGIN_MFLD_EX600_MODEL.Sup_Exh_Block_U_Side':
